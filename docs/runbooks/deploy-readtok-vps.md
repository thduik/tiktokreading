...
9. Typecheck and build `@workspace/readtok` with a fresh `READTOK_APP_VERSION`.
10. Emit `version.json` for client-side app freshness checks.
11. Write `runtime-config.js` with Clerk runtime keys.
12. Publish built frontend to `/var/www/readtok`.
13. Install the repo-managed Nginx site config with cache rules.
14. Reload Nginx and restart the API service.
15. Verify API health, `index.html`, `runtime-config.js`, `version.json`, and that the live bundle path matches the published bundle path exactly.
...
The client checks `/version.json` on startup, tab focus, and once per minute
with `cache: "no-store"`. If the deployed version differs from the version
embedded in the current JS bundle, the app reloads with `?appVersion=<version>`
so browsers with sticky HTML cache fetch the newest bundle reference.
...
curl -fsSL 'https://ieltstok.online/version.json?t=manual-check'
```
...
5. `version.json` contains the deploy app version generated by the deploy script.
6. The live API responds after restart.
7. If the task touched data, verify the production DB or API payload, not just local code.
